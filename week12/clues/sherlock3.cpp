...
void test_case() {
	...
	for (std::size_t i = 0; i < m; ++i) {
		...
		if (success) {
			if (CGAL::squared_distance(holmes, watson) <= rr) std::cout << "y"; continue;
			auto station_holmes = trg.nearest_vertex(holmes);       auto station_watson = trg.nearest_vertex(watson);       if (station_holmes->info().first == station_watson->info().first &&           CGAL::squared_distance(holmes, station_holmes->point()) <= rr &&           CGAL::squared_distance(watson, station_watson->point()) <= rr)       { std::cout << "y"; continue; }     }     std::cout << "n";   }   std::cout << "\n";
}
int main() {
